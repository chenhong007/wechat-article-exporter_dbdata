...
type AppmsgBarDataLike = Partial<{
  read_num: number;
...
type CgiDataLike = Partial<{
  user_info: Partial<{
    appmsg_bar_data: AppmsgBarDataLike;
  }>;
}>;

function extractCgiDataScriptFromHtml(html: string): string | null {
  try {
    const parser = new DOMParser();
    const doc = parser.parseFromString(html, 'text/html');
    const scripts = Array.from(
      doc.querySelectorAll('script[type="text/javascript"][h5only], script[h5only], script[type="text/javascript"], script')
    );
    const target = scripts.find(el => {
      const code = el.textContent || '';
      return code.includes('window.cgiDataNew = ') || code.includes('window.cgiData = ');
    });
    return target?.textContent?.trim() || null;
  } catch (e) {
    console.warn('提取 cgiData 脚本失败:', e);
    return null;
  }
}

async function parseCgiDataFromHtml(html: string): Promise<CgiDataLike | null> {
  // 仅在浏览器端解析：通过 iframe 隔离执行 HTML 中的目标脚本，拿到 window.cgiDataNew / window.cgiData
  if (!process.client || typeof document === 'undefined') return null;

  const code = extractCgiDataScriptFromHtml(html);
  if (!code) return null;
  ...
  return await new Promise(resolve => {
    ...
    iframe.onload = () => {
      try {
        ...
        const data = (w && (w.cgiDataNew || w.cgiData || w.window?.cgiDataNew || w.window?.cgiData)) as CgiDataLike | undefined;
        cleanup();
        resolve(data || null);
      } catch (e) {
        cleanup();
...
export class Downloader extends BaseDownload {
  ...
  private async processHtmlMetadata(blob: Blob, url: string): Promise<void> {
    const html = await blob.text();
    const cgiData = await parseCgiDataFromHtml(html);
    ...
    let likeNum = 0;
    let commentNum = 0;
    const appmsgBarData = cgiData?.user_info?.appmsg_bar_data;

    if (appmsgBarData) {
      readNum = Number(appmsgBarData.read_num) || 0;
      oldLikeNum = Number(appmsgBarData.old_like_count) || 0;
      shareNum = Number(appmsgBarData.share_count) || 0;
      likeNum = Number(appmsgBarData.like_count) || 0;
      commentNum = Number(appmsgBarData.comment_count) || 0;
    } else {
      // 兜底：从 HTML 中用正则/DOM 提取（至少保证阅读量可用）
      const parser = new DOMParser();
      ...
      if (readNumNewMatchResult?.groups?.read_num_new) {
        readNum = parseInt(readNumNewMatchResult.groups.read_num_new, 10);
      } else if (readNumMatchResult?.groups?.read_num) {
        readNum = parseInt(readNumMatchResult.groups.read_num, 10);
      }

      // 图片分享、文本分享、视频分享等类型 - 通过 d.read_num 赋值获取
      if (readNum === 0) {
        const dReadNumMatch = html.match(
          /d\.read_num\s*=\s*xml\s*\?\s*getXmlValue\(['"]appmsgstat\.read_num\.DATA['"]\)\s*\*\s*1\s*:\s*['"](\d+)['"]\s*\*\s*1/
        );
        if (dReadNumMatch?.[1]) {
      ...
      // 点赞、分享、喜欢、留言：这里保留原来的 DOM 方式（解析不到时默认 0）
      oldLikeNum = Number(document.querySelector('#js_bar_oldlike_btn')?.textContent) || 0;
      shareNum = Number(document.querySelector('#js_bar_share_btn')?.textContent) || 0;
      likeNum = Number(document.querySelector('#js_bar_like_btn')?.textContent) || 0;
      commentNum = Number(document.querySelector('#js_bar_comment_btn')?.textContent) || 0;
    }
